...
public class EnvMonitorServiceImpl extends ServiceImpl<TblHistoryMomentMapper, TblHistoryMoment> implements EnvMonitorService {
    ...
    public JsonResult<List<ItemAlarmInfoDTO>> getItemsAlarmInfo(ItemAlarmInfoVO param) {
        if (CollectionUtils.isEmpty(param.getItemTypeCodeList())) {
            List<CodeAndNameDTO> itemTypeList = getItemTypeList(param.getSysCode());
            List<String> itemTypeCodeList = itemTypeList.stream().map(CodeAndNameDTO::getCode).collect(Collectors.toList());
            param.setItemTypeCodeList(itemTypeCodeList);
        }
    ...
    public EChartsVO getAlarmUnhandledStatistics(String sysCode, String buildingCodes, String areaCode, Boolean isHandled, String startTime, String endTime) {
        ...
        // 获取所有设备类别
        List<String> itemTypeCodeList = this.getItemTypeList(sysCode).stream().map(CodeAndNameDTO::getCode).collect(Collectors.toList());
        // 初始化数据
     ...
     */
    @Override
    public List<CodeAndNameDTO> getItemTypeList(String sysCode) {
        List<ParameterTemplateAndDetailDTO> parameterInfo = getParameterInfo();
        List<CodeAndNameDTO> resultList = new ArrayList<>(parameterInfo.size());
        parameterInfo.forEach(e -> {
            resultList.add(CodeAndNameDTO.builder().code(e.getItemTypeCode()).name(e.getItemTypeName()).build());
        });
        return resultList;
    }
    ...
    public List<ParameterTemplateAndDetailDTO> getParameterInfo() {
        ...
        List<ParameterTemplateAndDetailDTO> parameterTemplateAndDetails = itemAPI.listParameterDetail(itemTypeAndParameterTypeCodeList).getData();
        return parameterTemplateAndDetails.stream().filter(e -> (0 != e.getItemCount())).collect(Collectors.toList());
    }
